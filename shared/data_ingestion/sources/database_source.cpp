...
std::vector<std::string> DatabaseSource::identify_slow_queries() {
    ...
    try {
        ...
        for (const auto& row : long_running.rows) {
            auto pid_it = row.find("pid");
            auto duration_it = row.find("duration");
            auto query_it = row.find("query");
            auto state_it = row.find("state");

            if (pid_it == row.end() || query_it == row.end()) continue;

            int pid = pid_it != row.end() ? std::stoi(pid_it->second) : 0;
            std::string duration = duration_it != row.end() ? duration_it->second : "unknown";
            std::string query = query_it->second;
            std::string state = state_it != row.end() ? state_it->second : "unknown";
        ...
        for (const auto& row : io_intensive.rows) {
            auto query_it = row.find("query");
            auto total_io_it = row.find("total_io");
            auto io_per_call_it = row.find("io_per_call");
            auto calls_it = row.find("calls");

            if (query_it == row.end()) continue;

            std::string query = query_it->second;
            int64_t total_io = total_io_it != row.end() ? std::stoll(total_io_it->second) : 0;
            int64_t io_per_call = io_per_call_it != row.end() ? std::stoll(io_per_call_it->second) : 0;
            int64_t calls = calls_it != row.end() ? std::stoll(calls_it->second) : 0;
        ...
        for (const auto& row : missing_indexes.rows) {
            auto schema_it = row.find("schemaname");
            auto table_it = row.find("tablename");
            auto seq_scan_it = row.find("seq_scan");
            auto avg_seq_tup_it = row.find("avg_seq_tup");

            if (schema_it == row.end() || table_it == row.end()) continue;

            std::string schema = schema_it->second;
            std::string table = table_it->second;
            int64_t seq_scan = seq_scan_it != row.end() ? std::stoll(seq_scan_it->second) : 0;
            int64_t avg_seq_tup = avg_seq_tup_it != row.end() ? std::stoll(avg_seq_tup_it->second) : 0;
